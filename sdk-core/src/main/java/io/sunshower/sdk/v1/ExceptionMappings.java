...
public class ExceptionMappings implements ExceptionMapper<Throwable> {
  ...
  static {
    register(AuthenticationCredentialsNotFoundException.class, new AuthenticationFailedResponse());

    register(AccessDeniedException.class, new ForbiddenResponse());

    register(InvalidCredentialException.class, new AuthenticationFailedResponse());

    register(InvalidTokenException.class, new AuthenticationFailedResponse());

    register(IllegalArgumentException.class, new BadRequestResponse());

    register(NoResultException.class, new NoSuchElementException());

    register(java.util.NoSuchElementException.class, new NoSuchElementException());

    register(EntityNotFoundException.class, new NotFoundExceptionResponse());

    register(NotFoundException.class, new NotFoundExceptionResponse());
  }

  public static Set<Map.Entry<Class<? extends Throwable>, ExceptionResponse>> getMappings() {
  ...
  public Response toResponse(Throwable exception) {
    ...
    if (exceptionResponse == null) {
      logger.log(
          Level.WARNING,
          "Encountered an exception while processing REST request: {0}",
...
class BadRequestResponse implements ExceptionMappings.ExceptionResponse {
  ...
  public Response create(Throwable throwable) {
    return Response.status(Response.Status.BAD_REQUEST)
        ...
        .entity(new Message((throwable.getMessage())))
        .build();
  }
}
